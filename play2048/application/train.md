...
fix 角落奖励（之前没用log2，导致有问题，并且系数跨度太大）
- 对比
    - qrdqn-cnn/QRDQN_19: 之前的，用于对照（无效移动惩罚）
    - qrdqn-cnn/QRDQN_21: 无效移动惩罚、角落惩罚（系数1）
    - qrdqn-cnn/QRDQN_22: 无效移动惩罚、角落惩罚（系数0.1）
- 结论
    - 21最差（角落惩罚系数太大）、21和22接近。其中21在初期有优势，后期优势缩小。
